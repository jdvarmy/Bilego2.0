...
import { getMapList, uploadFileMap } from '../../store/medialibrarySlice/medialibrarySlice';
...
const AppMapModal = ({ open, onClose, selectedDate }: Props) => {
  ...
  const handleUpload = useCallback(() => {
    if (fileList.map && fileList.minimap) {
      dispatch(uploadFileMap(fileList));
  ...
  const handleAddMap = useCallback(() => {
    if (selected && selectedDate) {
      ...
      !!uid && dispatch(editEventDate(uid, { ...data, type: TicketType.map, map: { uid: selected } }));
    }
    onClose();
    ...
    <ModalDialog
      ...
      contentActions={
        <Grid container sx={{ pt: 1 }}>
          ...
          <Grid item xs={3} onClick={() => setFileList(initialFileList)}>
            search
          </Grid>
        </Grid>
      }
