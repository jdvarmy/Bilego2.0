...
import { fetchArtistListForEvent } from '../../api/requests';
...
export const getArtistListForEvent =
  ...
  async (dispatch) => {
    ...
    try {
      await delayRandomly();
      const { data } = await fetchArtistListForEvent({ search }, { signal: abortController.signal });
